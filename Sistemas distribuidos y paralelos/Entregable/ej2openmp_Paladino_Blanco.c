...
void multyminAmaxD(double* a, double* b, double* d, double* c, double* r1, double* r2, double* min, double* max, int n, int bs){
  ...
  #pragma omp parallel for private(I,J,K,i,j,k,temp,temp2,temp3,temp4,ablk,bblk,cblk,dblk,r1blk,r2blk) reduction(max:maxloc) reduction(min:minloc)
  for(I = 0; I < n; I += bs)
  {
    temp3 = I*n;
    for(J = 0; J < n; J += bs)
...
void mult(double* mt1, double* c, double* mt2, double* b, double* r1, double* r2, int n, int bs){
  ...
  #pragma omp parallel for private(I,J,K,i,j,k,temp,temp2,temp3,temp4,mt1blk,mt2blk,cblk,bblk,r1blk,r2blk)
  for(I = 0; I < n; I += bs)
  {
    temp3 = I*n;
    for(J = 0; J < n; J += bs)
...
int I, J,aux;
double suma=0;
  #pragma omp parallel for private(I,J,aux) reduction (+:suma)
  for(I = 0; I < n; I += 1)
  {
      aux=I*n;
      for(J = 0; J < n; J += 1)
