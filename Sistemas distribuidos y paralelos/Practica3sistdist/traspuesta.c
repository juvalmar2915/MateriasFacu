...
{ 
 ...
 timetick = dwalltime();
 #pragma omp for schedule(dynamic,2) private(i,j,temp) nowait //ocurre que antes se le da mucho trabajo a ciertos hilos por lo que hay mucho desbalance de carga para eso se agrega la sentencia dynamic
  for(i=0;i<N;i++){
   for(j=i+1;j<N;j++){
		temp = A[i*N+j];
		A[i*N+j]= A[j*N+i];
